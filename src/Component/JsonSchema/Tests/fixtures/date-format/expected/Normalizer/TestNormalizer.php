...
class TestNormalizer implements DenormalizerInterface, NormalizerInterface, DenormalizerAwareInterface, NormalizerAwareInterface
{
    ...
    public function denormalize(mixed $data, string $type, ?string $format = null, array $context = []): mixed
    {
        ...
        if (\array_key_exists('date', $data)) {
            ...
            if (false === $date) {
                throw new \InvalidArgumentException(sprintf('Invalid date value "%s", expected format "%s".', $data['date'], 'd.m.Y'));
            }
        ...
        if (\array_key_exists('dateOrNull', $data) && $data['dateOrNull'] !== null) {
            ...
            if (is_string($data['dateOrNull']) and false !== \DateTime::createFromFormat('d.m.Y', $data['dateOrNull'])->setTime(0, 0, 0)) {
                ...
                if (false === $date_1) {
                    throw new \InvalidArgumentException(sprintf('Invalid date value "%s", expected format "%s".', $data['dateOrNull'], 'd.m.Y'));
                }
        ...
        if (\array_key_exists('dateOrNullOrInt', $data) && $data['dateOrNullOrInt'] !== null) {
            ...
            if (is_string($data['dateOrNullOrInt']) and false !== \DateTime::createFromFormat('d.m.Y', $data['dateOrNullOrInt'])->setTime(0, 0, 0)) {
                ...
                if (false === $date_2) {
                    throw new \InvalidArgumentException(sprintf('Invalid date value "%s", expected format "%s".', $data['dateOrNullOrInt'], 'd.m.Y'));
                }
