...
class BookJsonldNormalizer implements DenormalizerInterface, NormalizerInterface, DenormalizerAwareInterface, NormalizerAwareInterface
{
    ...
    {
        return $type === \ApiPlatform\Demo\Model\BookJsonld::class;
    }
    public function supportsNormalization(mixed $data, ?string $format = null, array $context = []): bool
    {
        return is_object($data) && get_class($data) === \ApiPlatform\Demo\Model\BookJsonld::class;
    }
    public function denormalize(mixed $data, string $type, ?string $format = null, array $context = []): mixed
    {
        ...
        if (\array_key_exists('archivedAt', $data) && $data['archivedAt'] !== null) {
            ...
            if (false === $date_1) {
                throw new \InvalidArgumentException(sprintf('Invalid datetime value "%s", expected format "%s".', $data['archivedAt'], 'Y-m-d\TH:i:sP'));
            }
    ...
    {
        return [\ApiPlatform\Demo\Model\BookJsonld::class => false];
    }
}